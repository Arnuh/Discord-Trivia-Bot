...
class Game extends EventEmitter {
  ...
  constructor(reply, GameHandler, channelId, groupID, ownerID, options, gameMode) {
    super();
  ...
  /**
   * Retrieves a config value pertaining to the game.
   * 
   * @param {*} val - Config value to get.
   * @returns {any} - The value of the requested config option.
   */
  getConfig(val) {
  ...
  /**
   * Send a message to users playing the game.
   * 
   * @param {*} str - The message to send.
   */
  broadcast(str) {
    this.emit("game_msg", {content: str});
  }

  /**
   * Ends the game visibly and destroys it.
   * Will emit game_end with an empty string if mid-round (handled by round_end instead).
   * If between rounds, will emit game_end with the string, if any.
   * 
   * @param {string} [endStr] - The message to end the game with.
   */
  async endGame(endStr) {
    this.cancelled = true;
    
    if(this.inRound) {
      // Call endRound if in a round (This will emit round_end)
      await this.endRound(endStr);
      this.emit("game_end");
    }
    else if(endStr !== undefined) {
      const str = this.gameStrings.buildEndStr(this, true, true, endStr);
      this.emit("game_end", str);
    } else {
      this.emit("game_end");
    }
  ...
  /**
   * Interprets the game's current answers into an array of answers.
   * 
   ...
   * 
   * @returns {string} The list of answers for display to users.
   */
  buildAnswers() {
    var answerString = "";
    var answers = [];
  ...
  /**
   * Converts a length in ms to a time unit of hours or minutes.
   * 
   * @param {number} input - Length in milliseconds.
   * @returns {string} The string for display to users.
   */
  getTimerStr(input) {
    var timerStr = "";
    var numStr;
   ...
   * 
   * @returns {str} The string for the game to send to players.
   */
  // initializeRound
  // 
  async initializeRound() {
    ...
    try {
      ...
      const reply = (msg) => {
        this.emit("game_msg", msg);
      };

      question = await this.Trivia.getTriviaQuestion(reply, 0, this.ID, 0, isFirstRound, this.options.category, this.options.type, this.options.difficulty);
      ...
      if(typeof this.Trivia.maintenanceMsg === "string") {
        this.emit("game_error", this.Trivia.maintenanceMsg);
      }
      else {
    ...
    var infoString = this.gameStrings.buildInfoString(this);
  ...
  submitAnswer(userId, username, isCorrect) {
    ...
    if(isCorrect) {
      if(this.getConfig("only-one-winner") && Object.keys(this.usersCorrect).length > 0){
        return;
      }
  ...
  /**
   * Returns whether the game should end at the end of this round.
   *
   * @returns {boolean} Whether the game is ending at the end of the current round.
   */
  getIsEnding() {
    if(this.cancelled) {
      return true;
   ...
   * 
   * @param {*} endStr - Additional message to end the round with.
   * @returns {void}
   */
  async endRound(endStr) {
    ...
    var str = this.gameStrings.buildEndStr(this, gameIsEnding, false, endStr);
    ...
    if(gameIsEnding) {
      ...
      // Make sure we haven't already begun ending the game
      // (i.e. in a manually cancelled game where endGame is called)
      if(!this.cancelled) {
        this.endGame();
      }
      return;
